  ...
  const ingredientBlock = evidence
    .map(({ ingredient, articles, abstractsText, riskyNote, doseAssessment }) => {
      ...
      const articlesLine = articles.length > 0
        ? `Related PubMed articles:\n${articles.map((a) => `- [PMID ${a.pmid}, ${a.year ?? "n.d."}] ${a.title}`).join("\n")}`
        : "No directly relevant PubMed articles found in this search.";
...
- Ground every ingredient's "evidenceGrade" and "note" in the PubMed evidence given below when available; use "insufficient" honestly when there isn't enough.
- Never state adverse-event counts as proof of causation — describe them as unverified reports.
